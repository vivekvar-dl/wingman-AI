...
export class SubjectDetector {
  private subjects: SubjectPattern[] = [
    {
      name: 'mathematics',
      keywords: [
  ...
  public detectSubject(text: string, extractedText?: string): DetectionResult {
  ...
  private generateSuggestions(subject: string): string[] {
  ...
  public enhancePromptWithSubject(
    basePrompt: string, 
  ...
  public getSubjectSpecificShortcuts(subject: string): Array<{key: string, description: string, action: string}> {
    const shortcuts: { [key: string]: Array<{key: string, description: string, action: string}> } = {
      mathematics: [
