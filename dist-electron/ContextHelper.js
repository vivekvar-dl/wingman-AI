...
class ContextHelper {
    ...
    async getActiveApplication() {
        try {
            ...
            if (process.platform === 'win32') {
                ...
                activeApp = (0, child_process_1.execSync)(command, { encoding: 'utf8' }).trim();
            }
            else {
    ...
    parseWindowsOutput(output) {
        try {
            ...
            for (const line of lines) {
                if (line.includes('ProcessName') || line.includes('---'))
                    continue;
    ...
    getContextualShortcuts(context) {
        ...
        switch (context.contentType) {
            case 'code':
                shortcuts.push({ key: 'Ctrl+Shift+E', description: 'Explain Code', action: 'explain-code' }, { key: 'Ctrl+Shift+D', description: 'Debug Error', action: 'debug-error' }, { key: 'Ctrl+Shift+O', description: 'Optimize Code', action: 'optimize-code' });
                break;
            case 'math':
                shortcuts.push({ key: 'Ctrl+Shift+S', description: 'Solve Equation', action: 'solve-math' }, { key: 'Ctrl+Shift+G', description: 'Graph Function', action: 'graph-function' });
                break;
            case 'exam':
                shortcuts.push({ key: 'Ctrl+Shift+Q', description: 'Quick Help', action: 'quick-help' }, { key: 'Ctrl+Shift+M', description: 'Multiple Choice', action: 'mc-help' });
                break;
        }
        return shortcuts;
    }
